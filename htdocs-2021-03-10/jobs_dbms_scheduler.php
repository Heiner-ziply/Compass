...
$headline='<h1>Oracle DBMS_SCHEDULER job directory</h1>';
...
$sql = "select replace(global_name,'.WORLD','') global_name,owner,job_name,what,last_started,next_run,schedule_name,
comments,nvl(upd_date,ins_date) poll_date from overdrive.global_scheduler_jobs
where owner not in ('SYS','SYSTEM','ORACLE_OCM')
order by global_name, owner,job_name";
